...
if ( has_nav_menu( 'language-footer-menu' ) ) { // Check to see if there is a footer menu assigned.
	$menu_id   = $menu_locations['language-footer-menu']; // Get the *language-footer-menu* menu ID.
	$menu_item = wp_get_nav_menu_items( $menu_id );
	if ( $menu_item ) { // Get the array of wp objects, the nav items for our queried location.
		?>
		<h2 class="govuk-visually-hidden">Other languages</h2>
		...
		<ul class="govuk-footer__inline-list">
          ...
          <li class="govuk-footer__inline-list-item">
            <a class="govuk-footer__link" lang="' . esc_html( $nav_item->lang ) . '" href="' . esc_url( $nav_item->url ) . '">' . esc_html( $nav_item->title ) . '</a>
          </li>';
			}
		...
		<?php
	} //end if language-footer-menu exists
