...
if ( ! function_exists( 'hale_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function hale_posted_on() {

        echo '<time class="entry-date published-date" datetime="' . get_the_date( DATE_W3C ) . '">' . get_the_date() . '</time>';

	}
endif;

if ( ! function_exists( 'hale_posted_by' ) ) :
	/**
...
if ( ! function_exists( 'hale_entry_footer' ) ) :
	 ...
	 */
	function hale_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'hale' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<p class="cat-links">' . esc_html__( 'Posted in %1$s', 'hale' ) . '</p>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'hale' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<p class="tags-links">' . esc_html__( 'Tagged %1$s', 'hale' ) . '</p>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					...
					/* translators: %s: post title */
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'hale' ),
						array(
							'span' => array(
								'class' => array(),
							),
...
if ( ! function_exists( 'hale_post_thumbnail' ) ) :
	/**
	 * Displays an optional post thumbnail.
	 * Wraps the post thumbnail in an anchor element on index views, or a div
	 * element when on single views.
	 */
	function hale_post_thumbnail() {
 ...
 */
function hale_comment_display( $comment, $args, $depth ) {
	if ( 'div' === $args['style'] ) {
		$tag       = 'div';
		$add_below = 'comment';
	} else {
	...
	<div class="comment-author vcard">
		...
		printf( __( '<cite class="fn">%s</cite>', 'hale' ), get_comment_author_link() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		?>
	...
	edit_comment_link( hale_edit_icon(), '  ', '' );
	echo '</div>';
	if ( 'div' !== $args['style'] ) :
		echo '</div>';
	endif;
 ...
 */
function hale_comment_reply_text( $link ) {
	$link = str_replace( '>Reply', hale_reply_icon(), $link );

	return $link;
}

add_filter( 'comment_reply_link', 'hale_comment_reply_text' );
 ...
 */
function hale_remove_image_size_attributes( $html ) {
	return preg_replace( '/(width|height)="\d*"/', '', $html );
}

// Remove image size attributes from post thumbnails.
add_filter( 'post_thumbnail_html', 'hale_remove_image_size_attributes' );

// Remove image size attributes from images added to a WordPress post.
add_filter( 'image_send_to_editor', 'hale_remove_image_size_attributes' );
